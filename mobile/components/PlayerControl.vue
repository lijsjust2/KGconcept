                        ...
                        <transition name="cover-fade" mode="out-in">
                            <div v-if="coverMode === 'vinyl'" key="vinyl" class="vinyl-player">
                                ...
                                <div class="vinyl-disc" :class="{ 'rotating': playing }">
                                    <img v-if="currentSong?.img" :src="currentSong.img" alt="Album Art" class="vinyl-cover" />
...
const handleLoudnessChange = (event) => {
    ...
    if (DEV) console.log('[PlayerControl] 响度规格化开关变更:', enabled);
    toggleLoudnessNormalization(enabled);
};
const audioPauseHandler = () => {
    playing.value = false;
    if (DEV) console.log('[PlayerControl] 暂停事件');
...
const audioPlayHandler = () => {
    playing.value = true;
    if (DEV) console.log('[PlayerControl] 播放事件');
    if (!lyricsData.value.length) getCurrentLyrics();
    if (isElectron()) window.electron.ipcRenderer.send('play-pause-action', playing.value, audio.currentTime);
};
const audioErrorHandler = (e) => {
    if (DEV) console.log('[PlayerControl] 音频错误代码:', audio.error?.code);
...
onMounted(() => {
    ...
    audio.addEventListener('pause', audioPauseHandler);
    audio.addEventListener('play', audioPlayHandler);
...
onUnmounted(() => {
    ...
    // 清理组件特定的监听器
    audio.removeEventListener('pause', audioPauseHandler);
    audio.removeEventListener('play', audioPlayHandler);
